...
//这边提取页面信息，存入数据库
@Component("JobUrlDaoPipeline")
public class JobUrlDaoPipeline implements PageModelPipeline<JobUrlBean> {
    ...
    @Override
    public void process(JobUrlBean jobBean, Task task) {
        //查数据库看是否已经有这个职位了
        ...
        if (id == null) {
            ...
            //得到职位名称
            positionEntity.setpName(jobBean.getJobname().replaceAll(" ", ""));
            //对薪水处理，将字符串转化为double
...
//        }
            ...
            //这里对Cname处理
            String Cname = jobBean.getCname().replaceAll(" ", "");
//            if(Cname.charAt(0)==' '){
//                Cname=Cname.substring(1);
//            }
            //这边最后是根据url判断
            Integer c = cUrlEntityMapper.selectByUrl(jobBean.getCompanyurl());
            //Integer c = companyEntityMapper.selectbyName(Cname);
            //System.out.println(Cname);
            //System.out.println(c);
            //System.out.println(jobBean.getCname().replaceAll(" ",""));
            //判断公司是否在数据库中，然后处理对应的逻辑
            if (c != null) {
...
//            positionEntity.setPMinware(10000.0);
    ...
    public List<String> selcet(int start, int end) {
        List<String> list = urlEntityMapper.selectNum(start, end);
//        String url = "";
//        //System.out.println(url);
//        for (int j = 0; j < list.size() - 1; j++) {
//            url = url + "\""+list.get(j)+"\"" + ",";
//        }
//        url = url +"\""+ list.get(list.size() - 1)+"\"";
//        System.out.println(url);
        return list;
    }
